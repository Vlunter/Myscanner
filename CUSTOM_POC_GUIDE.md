...
### v1.1 - 第一批优化 (2026-06-15)

> 本次更新新增 **3大核心功能**，大幅提升扫描器的实用性。

---

#### 新增功能一：扫描报告导出

**新增模块**: [scanners/report_generator.py](scanners/report_generator.py)

支持 **3种格式** 的报告自动生成：

| 格式 | 特点 | 适用场景 |
|------|------|---------|
| **HTML** | CSS样式 + 统计柱状图 + 颜色徽章 + 完整面板布局 | 浏览器查看、邮件发送、审计交付 |
| **JSON** | 结构化数据、字段齐全、程序可读 | API对接、二次处理、自动化流水线 |
| **TXT** | 纯文本对齐格式、终端友好 | 终端阅读、打印归档 |

**使用方式：**

每次扫描完成后（模式3/4/6），程序会自动询问是否生成报告：

```
[?] 是否生成扫描报告? (1=HTML / 2=JSON / 3=TXT / 4=全部 / n=跳过): 
```

选择后报告自动保存到 `reports/` 目录：
- HTML: `reports/scan_report_target_20260615_140000.html` （可直接浏览器打开）
- JSON: `reports/scan_report_target_20260615_140000.json`
- TXT: `reports/scan_report_target_20260615_140000.txt`

**HTML报告包含的板块：**

```
┌─────────────────────────────────────┐
│  Myscanner 安全扫描报告              │  ← 深色渐变头部 + 目标信息
├─────────────────────────────────────┤
│  [5] [2] [3] [0]                    │  ← 统计卡片(总数/Critical/High/Medium/Low)
│                                     │
│  ████░░                              │  ← 严重度分布柱状图
│  ██████░                             │
│  ████████                            │
...
# 方式B: 直接粘贴多个目标（逗号/空格/换行分隔）
192.168.1.1, example.com, test.org, http://target.com
```
...
```
请选择批量扫描内容:
    1. 全部扫描 (port + finger + vuln + headers) [推荐]
    2. 仅漏洞检测 (vuln)
    3. 仅端口扫描 (port)
    4. 仅安全头检测 (headers)
...
[?] 扫描完成后是否生成HTML报告? (y/n，默认y): y
```

**智能防护机制：**
- 目标间自动延迟 **1~3秒随机间隔**（防止被目标封禁IP）
- 单个目标失败不影响后续（自动跳过并计数）
- 支持 **Ctrl+C 中断当前目标**，继续扫描下一个
- 批量完成后自动输出 **汇总统计** + **合并HTML报告**
...
[+] 报告已保存: reports\batch_scan_20260615_143000.html
[+] 报告已保存: reports\batch_scan_20260615_143000.json
```
...
**新增模块**: [scanners/security_headers.py](scanners/security_headers.py)

**新增入口**: 主菜单选择 `6`（独立模式），或包含在模式4（全扫）中自动执行

**12项安全头全覆盖检测：**

| # | 安全头名称 | 缺失风险等级 | 缺失后果 | CWE编号 |
|---|-----------|:----------:|---------|--------|
| 1 | **X-Frame-Options** | High | 点击劫持(Clickjacking)攻击 | CWE-1021 |
| 2 | **Content-Security-Policy** | **Critical** | XSS跨站脚本、数据注入攻击 | CWE-79 |
| 3 | **Strict-Transport-Security** | High | SSL剥离攻击、中间人劫持 | CWE-319 |
| 4 | **X-Content-Type-Options** | Medium | MIME类型嗅探导致混淆 | CWE-16 |
| 5 | **Referrer-Policy** | Low | URL路径和参数泄露给第三方 | CWE-200 |
| 6 | **Permissions-Policy** | Low | 恶意网站调用摄像头/麦克风等设备API | CWE-200 |
| 7 | **X-XSS-Protection** | Medium | 旧版浏览器缺少XSS过滤层 | CWE-79 |
| 8 | **Server** | Low | 暴露服务器精确版本号便于针对性利用 | CWE-200 |
| 9 | **X-Powered-By** | Low | 暴露后端技术栈细节(PHP/ASP.NET/Express) | CWE-200 |
| 10 | **Cache-Control** | Medium | 敏感页面被缓存到磁盘导致信息泄露 | CWE-524 |
| 11 | **Access-Control-Allow-Origin** | High | CORS通配符*导致跨域数据窃取 | CWE-942 |
| 12 | **Cookie-Security**(HttpOnly/Secure/SameSite) | High | Cookie被JS读取或明文传输 | CWE-614/319/1004 |

**智能判断逻辑：**

每项安全头不仅检查"有无"，还会深入分析值的质量：

```
Content-Security-Policy:
    ✓ 有CSP头 → 进一步检查是否含 unsafe-inline/unsafe-eval → 给出优化建议
    
Strict-Transport-Security:
    ✓ 有HSTS头 → 解析max-age值 → < 31536000(1年)标记为WEAK → 建议延长
    
Access-Control-Allow-Origin:
    ✓ 有CORS头 → 值为 * → 标记为RISKY → 建议指定明确域名
                → 值为 null → 标记为RISKY → 可能被绕过
```
...
请选择扫描模式:
    ...
    3. 漏洞检测 (vuln)
    4. 全部扫描 (all) [推荐]
         └─ 包含: 端口 + 指纹 + 漏洞 + 安全头 + 报告提示
    5. 漏洞库管理 (db) [POC管理/CVE查询]
    6. 安全头检测 (headers)          ★ v1.1新增
    b. 批量扫描 (batch) [多目标]      ★ v1.1新增
         └─ 支持文件导入 / 粘贴输入 / 自动汇总报告
```
...
| 文件 | 操作 | 说明 |
|------|:----:|------|
| `scanners/report_generator.py` | **新增** | HTML/JSON/TXT 三格式报告生成器 |
| `scanners/security_headers.py` | **新增** | 12项HTTP安全头检测 + 评分系统 |
| `main.py` | **修改** | +批量扫描 +安全头模式6 +报告提示 |
| `.gitignore` | **修改** | +排除 reports/ 目录 |
| `Myscanner.spec` | **修改** | +2个新模块打包配置 |

---

*最后更新: 2026-06-15 | Myscanner v1.1*
